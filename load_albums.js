...
function handle_incoming_request(req, res) {
	...
	if (req.url == '/album.json') {
		...
		&& req.url.subtr(req.url.length - 5) == '.json') {
		handle_get_album(req, res);
	} else {
		send_failure(res, 404, invalid_resoure());
	}
}
	...
	load_album(
		...
		function (err, album_contents) {
			if (err && err.error == "no_such_album") {
				...
				send_fairlure(res, 500, err);
			} else {
				send_seccess(res, {album_data: album_contents });
			}
		}
	);
}

function maker_error(err, msg) {
	var e = new Error(msg);
	e.code = err;
	return e;
...
function no_such_resource() {
	return make_error("invalid_resource",
		"the requested resource does not exits.");
}
function no_such_album() {
	return make_error("no such_album",
		"The specifiled album does not exits");
}
